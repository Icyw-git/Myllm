...
$$
\mathcal{L}=-\frac{1}{B}\sum_b \log p(y_b\mid x_b)
$$
...
| 阶段 | 条件 | 行为 |
|------|------|------|
| Warmup | `it < Tw` | 从 0 **线性**爬到 \(\alpha_{\max}\) |
| Cosine | `Tw ≤ it ≤ Tc` | 半个余弦从 \(\alpha_{\max}\) **平滑降到** \(\alpha_{\min}\) |
| 平台 | `it > Tc` | 恒为 \(\alpha_{\min}\) |
...
3. **落到 \(\alpha_{\min}\) 而非 0**：还能微调，不完全停学。
