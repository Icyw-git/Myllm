...
created: 2026-07-24
updated: 2026-07-26
source: assignment1/assignment1-basics/cs336_basics/linear.py；train_bpe.py
---

# CS336 A1 · 模型层手撕与踩坑

> 对应代码：`assignment1/assignment1-basics/cs336_basics/linear.py`、`train_bpe.py`  
...
### `angles = positions.unsqueeze(-1) * rope_theta` 怎么对齐

PyTorch 广播**从右边对齐**：

| | 形状 | 含义 |
|---|---|---|
| `token_positions` | `(..., S)` | 每个 token 的位置 |
| `.unsqueeze(-1)` | `(..., S, 1)` | 给频率维留个可广播的 1 |
| `rope_theta` | `(d_k//2,)` | 每对维度一个 ω |
| **乘积 `angles`** | `(..., S, d_k//2)` | `φ[s,i] = pos[s] * ω[i]` |

最右维：`1` 对 `d_k//2` → 扩成 `d_k//2`；左边的 batch/seq 维原样保留。  
这是「位置 × 频率」的外积，和后面 MHA 里再乘 Q 是**另一步**广播。
...
def rope(
    ...
    *prefix, seq_len, d_k = in_query_or_key.shape
    ...
    # (...,S) → (...,S,1) * (d_k//2,) → (...,S,d_k//2)；从右边对齐
    angles = token_positions.unsqueeze(-1) * rope_theta
    cos = angles.cos()
    sin = angles.sin()

    # 成对 (x0,x1),(x2,x3),...；此时 x 与 cos 都是 (...,S,d_k//2)
...
> [!failure] 错过2：`cos` 缺 head 维（训练时 B≠H 才炸）
> `angles` 得到 `(B,S,d_k/2)` 后若直接乘 `Q[...,0]=(B,H,S,d_k/2)`，从右边对齐会变成：
>
> ```text
> Q:   B, H, S, d_k/2
> cos:    B, S, d_k/2   ← dim1 用 H 去对 B
> ```
>
> 测试 fixture 常 `B==H==4` 碰巧过；训练 `B=8,H=16` 报  
> `size a (16) must match b (8) at non-singleton dimension 1`。  
> **正确**：`cos/sin.unsqueeze(1)` → `(B,1,S,d_k/2)`，在 head 维广播。
...
def multihead_self_attention_with_rope(
    ...
    *prefix, seq_len, d_model = in_features.shape
    ...
    # ① (B,S,1)*(d_k/2,) → (B,S,d_k/2)   同 rope 的 angles 对齐
    # ② unsqueeze(1) → (B,1,S,d_k/2)       才能乘 (B,H,S,d_k/2)
    freq_seq = torch.arange(0, d_k, 2, dtype=in_features.dtype, device=in_features.device) / d_k
    rope_theta = 1 / theta**freq_seq
    angles = token_positions.to(in_features.device).unsqueeze(-1) * rope_theta
...
对应：`linear.py` → `get_batch`；测试 `test_get_batch`。

### 在算什么

从 1D token 数组里随机采 `(x, y)`：`x` 长 `context_length`，`y` 是右移 1 位的下一步标签，形状都是 `(batch_size, context_length)`。

### 我的错误 / 注意

> [!failure] 错过：`max_start = len - context_length` 且 `randint(0, max_start+1)`
> 会抽到 `start = len - cl`。此时 `x` 长 `cl`，`y = dataset[start+1 : start+cl+1]` 只有 `cl-1`。  
> `torch.tensor` 拼 batch 时报：`expected sequence of length 6 at dim 1 (got 7)`。  
> **正确**：窗口要 `cl+1` 个 token；合法 start 是 `0 .. len-cl-1`。  
> 写法：`max_start = len - cl`，`np.random.randint(0, max_start, ...)`（上界不含）。

> [!failure] 错过：`torch.tensor(list_of_ndarrays)`
> 慢，且长度不一致时错误难读。  
> **正确**：先 `np.stack(..., axis=0)` 得 `(B, cl)`，再 `torch.from_numpy`。

```python
def get_batch(dataset: npt.NDArray, batch_size: int, context_length: int, device: str):
    # 错过：randint(0, max_start+1) 会抽到非法 start → x/y 长度不一致
    # 正确：合法 start ∈ [0, len-cl)；窗口实际需要 cl+1 个 token
    ...
    # 错过：torch.tensor(list_of_ndarrays)；正确：stack 后再 from_numpy
    inputs = np.stack(inputs, axis=0)    # (B, cl)
    outputs = np.stack(outputs, axis=0)  # (B, cl)
...
对应：`train_bpe.py` → `BPE.train`（adapter `run_train_bpe`）；测试 `test_train_bpe*`。

> [!warning] 改对文件
> 曾把增量写在 `train_bpe.py`，但 adapter 一度指向 `get_tokenizer.train_bpe1` → **改了也不测**。  
> speed 仍 ~3s。确认 `tests/adapters.py` 里 `run_train_bpe` 调的是你正在改的实现。

### 在算什么

Pretokenize → 字节词表 → 反复选最高频相邻 pair merge，直到 `vocab_size`。  
`test_train_bpe_speed`：`corpus.en` + `vocab=500` 必须 **< 1.5s**（naive 全量重扫约 3s）。

### 我的错误 / 注意

> [!failure] 错过：词表 / pretoken 用错类型
> - `vocab[i] = chr(i)`（str）→ 测试要 **bytes**，应 `bytes([i])`  
> - special 直接存 str → 应 `token.encode("utf-8")`  
> - `list(word.encode(...))` 得到 **int**（32, 116）→ merges 变成 `(32,116)`，且 `+` 是加法不是拼接  
> - **正确**：序列元素是长度为 1 的 `bytes`，如 `bytes([b])`；merge 时 `b' ' + b't' -> b' t'`

> [!failure] 错过：`merges = []` 放进 `while` 里
> 每轮清空，最后只剩 1 条 merge。  
> **正确**：`merges` 在循环外初始化，每轮 `append`。

> [!failure] 错过：`max(pairs, key=pairs.get)` 平局不分
> 频次相同时只按遍历顺序，会和 reference 分叉（如 index 64: `(b'c',b'e')` vs `(b'l',b'e')`）。  
> **正确**：`max(..., key=lambda x: (pair_counts[x], x))`，平局按 pair 字典序。

> [!failure] 错过：`NameError: defaultdict is not defined`
> 用了 `defaultdict(set)` 却只 `from collections import Counter`。  
> **正确**：`from collections import Counter, defaultdict`。  
> `defaultdict(set)`：key 不存在时自动空 set，可直接 `.add`，省 `if key not in d`。

> [!failure] 错过：`RuntimeError: Set changed size during iteration`
> `for tokens in pair_to_words[best_pair]:` 循环里又对同一 set `remove`/`add`。  
> **正确**：`affected = list(pair_to_words[best_pair])`，再 `for tokens in affected`。

> [!failure] 错过：naive 每轮扫全库（speed ~3s）
> 每轮 `Counter` 重数所有 pair + 重建整个 `word_freq` → $O(\#merges \times \#words)$。  
> **正确（增量）**：
> 1. 初始化算一遍 `pair_counts` + 倒排 `pair_to_words`
> 2. 只处理含 `best_pair` 的 word
> 3. 对每个受影响 word：**减旧序列全部 pair 贡献 → merge → 加新序列全部 pair 贡献**（都乘 `freq`）
> 4. 更新 `word_freq` / 倒排；`best_pair` 计数清掉后再选下一轮

> [!failure] 错过：增量更新时的若干实现坑
> - `word_freq = new_word_freq` 且 new **只含 affected** → 无关词全丢  
> - merge 用 `for i in range(len-1)` 只 append 左侧、忘 `i+=2` → 应用 `while i` 不重叠合并  
> - `pair_counts[p] -= 1` 应为 `-= freq`；且要动**邻接 pair**，不是只动 `best_pair`  
> - 倒排里存 `tuple(tokens)`，旧 key 要从相关 set 删掉，新 key 再加进去  
> - `list.remove` 慢且重复危险；倒排用 `set` + `discard` 更稳

```python
# 错过：from collections import Counter 却用了 defaultdict → NameError
from collections import Counter, defaultdict

# ... pretokenize / vocab 初始化略 ...

pair_counts = Counter()
pair_to_words = defaultdict(set)  # pair → 含该 pair 的 word(tuple) 集合
...
while len(vocab) < vocab_size:
    if not pair_counts:
        break
    # 平局：按 (频次, pair) 字典序，和 reference 对齐
    best_pair = max(pair_counts, key=lambda x: (pair_counts[x], x))
    merges.append(best_pair)
    vocab[len(vocab)] = best_pair[0] + best_pair[1]

    # 错过：直接 for tokens in pair_to_words[best_pair] → Set changed size
    # 正确：先拷贝
    affected = list(pair_to_words[best_pair])
    for tokens in affected:
        freq = word_freq[tokens]
        # 1) 减旧序列贡献的全部 pair（×freq），更新倒排
        # 2) while i 做不重叠 merge 得到 new_tokens
        # 3) 加新序列贡献的全部 pair（×freq），更新倒排
        # 4) word_freq[new] += freq; del word_freq[old]
        ...
    # best_pair 不再可选：计数置 0 或删掉
```

### 自测

```bash
uv run pytest -k 'test_train_bpe and not speed'   # 先对齐 reference
uv run pytest -k test_train_bpe_speed             # < 1.5s
```
...
| `size a (H) must match b (B) at dim 1`（RoPE） | cos 缺 head 维；要 `(B,1,S,·)` 再乘 Q |
...
| `expected sequence of length 6 ... (got 7)` | `get_batch` start 越界；`randint` 上界应不含 `len-cl` |
| BPE vocab / merges 类型错 | 用 `bytes([i])`，不要 `chr`/`int`；special 要 encode |
| BPE 某步和 reference 分叉 | 平局用 `(count, pair)`；`merges` 勿放进 while |
| `defaultdict` NameError | `from collections import Counter, defaultdict` |
| `Set changed size during iteration` | 先 `list(pair_to_words[best])` 再遍历 |
| `test_train_bpe_speed` ~3s | 仍在全量重扫；或改错文件（adapter 指向别处） |
| BPE 增量后和 reference 不一致 | 减/加要用 `freq`；邻接 pair 都要更新；merge 用 `i+=2` |
...
uv run pytest -k 'test_train_bpe and not speed' -v
...
> **数值**减 max；**形状**`//`/`keepdim`/`contiguous`；**mask**在 softmax 前且分清极性；**RoPE** $\theta^{-2i/d}$ 只转 Q/K；**Block** pre-norm + 外侧残差；**LM** 剥 `layers.{i}.` 成 dict；**AdamW** 返回类、`lr` 别乘两次；**cosine** 管全局日程，**Adam** 管按维自适应，二者叠加；**get_batch** 窗口 `cl+1`；**BPE** 增量更新 + 先拷贝再改 set。
