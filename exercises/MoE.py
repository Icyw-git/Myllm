    ...
    def forward(self, x, tokens=None, verbose=False):  # x:(batch,d_model)
        ...
        for i in range(self.k):
            ...
            for e_id, expert in enumerate(self.experts):
                ...
                mask = (expert_ids == e_id).unsqueeze(1)  # mask:(batch,1)

                if mask.sum() == 0:
                    continue

                expert_output += expert(x * mask)  # expert_output:(batch,d_model)

            out += expert_output * expert_weight.unsqueeze(1)  # out:(batch,d_model) 乘以对应的专家权重
...
class EC_MoE(nn.Module):
    ...
    def forward(self,x,tokens=None,verbose=False):  # x:(batch,d_model)
        ...
        topk_scores,topk_idx=scores.topk(min(self.k,batch),dim=-1)
        out=torch.zeros_like(x)  # out:(batch,d_model)
        for e in range(self.num_experts):
            token_scores=topk_scores[e]  # token_scores:(k,)
            token_idx=topk_idx[e]
            selected_tokens=x[token_idx]  # selected_tokens:(k,d_model)
            ...
            out[token_idx]+=weighted_output  # out:(batch,d_model) [k,d_model]+=[k,d_model]
